...
/* Hide Streamlit's form submit hint so it does not overlap compact inputs */
...
with st.sidebar:
    ...
    with st.form("analyse_form", clear_on_submit=True):
...
if run_btn:
    if not source.strip():
        ...
        st.session_state.pending_source = source.strip()
...
if st.session_state.processing and st.session_state.pending_source:
        ...
        try:
            ...
            progress_bar.progress(10, text="Preparing audio...")
            chunks = process_input(source)
            update_step("audio", "done")
            progress_bar.progress(25, text="Audio ready. Transcribing...")

            update_step("transcript", "active")
            transcript = transcribe_all(chunks)
            update_step("transcript", "done")
            progress_bar.progress(45, text="Transcript ready. Creating title...")

            update_step("title", "active")
            title = generate_title(transcript)
            update_step("title", "done")
            progress_bar.progress(55, text="Creating summary...")

            update_step("summary", "active")
            summary = summarize(transcript)
            update_step("summary", "done")
            progress_bar.progress(70, text="Extracting action items and decisions...")
            ...
            progress_bar.progress(85, text="Preparing chat with transcript...")
